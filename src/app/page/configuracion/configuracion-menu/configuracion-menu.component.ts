...
import { MenuService } from '../../../shared/service/service.index';

@Component({
  selector: 'app-configuracion-menu',
  templateUrl: './configuracion-menu.component.html',
  styleUrl: './configuracion-menu.component.css'
})
export class ConfiguracionMenuComponent {
  ...
  public async searchData() {
    ...
    var result = await this.menuService.getData(urlFilters);
  ...
  public async initDeleteForm(item: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let result = await this.menuService.deleteData(item.id);
        if (result.status === 200) {
  ...
  public async submitForm() {
    ...
    let result: any;
    if (this.isUpdate) { result = await this.menuService.putData(this._form.value.id, JSON.stringify(this._form.value)); }
    else { result = await this.menuService.postData(JSON.stringify(this._form.value)); }

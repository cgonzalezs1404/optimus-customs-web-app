...
@Component({
  ...
  templateUrl: './configuracion-privilegio.component.html',
  styleUrl: './configuracion-privilegio.component.css'
})
export class ConfiguracionPrivilegioComponent implements OnInit{
  ...
  public async searchData() {
    ...
    var result = await this.privilegioService.getData(urlFilters);
  ...
  public async initDeleteForm(item: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let result = await this.privilegioService.deleteData(item.id);
        if (result.status === 200) {
  ...
  public async submitForm() {
    ...
    let result: any;
    if (this.isUpdate) { result = await this.privilegioService.putData(this._form.value.id, JSON.stringify(this._form.value)); }
    else { result = await this.privilegioService.postData(JSON.stringify(this._form.value)); }

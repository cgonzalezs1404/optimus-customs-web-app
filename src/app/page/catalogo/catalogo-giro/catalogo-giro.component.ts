...
import { GiroService } from '../../../shared/service/giro.service';
import Swal from 'sweetalert2';

@Component({
  selector: 'app-catalogo-giro',
  templateUrl: './catalogo-giro.component.html',
  styleUrl: './catalogo-giro.component.css'
})
export class CatalogoGiroComponent implements OnInit {
  ...
  public async searchData() {
    ...
    var result = await this.giroService.getData(urlFilters);
  ...
  public async initDeleteForm(item: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let result = await this.giroService.deleteData(item.id);
        if (result.status === 200) {
  ...
  public async submitForm() {
    ...
    let result: any;
    if (this.isUpdate) { result = await this.giroService.putData(this._form.value.id, JSON.stringify(this._form.value)); }
    else { result = await this.giroService.postData(JSON.stringify(this._form.value)); }
